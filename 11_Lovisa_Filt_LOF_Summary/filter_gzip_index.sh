...
for i in /proj/data9/Loss_of_function_mutations/10_Lovisa_LOF_Bamboozle/*/*_lof.vcf.gz; do
...
#        GROUP="Mutant_"$dirname
	...
	echo "#$ -o /proj/data9/Loss_of_function_mutations/11_Lovisa_Filt_LOF_Summary/" >> $filename"_script.sge"
        echo -e "#$ -e /proj/data9/Loss_of_function_mutations/11_Lovisa_Filt_LOF_Summary/\n" >> $filename"_script.sge"
...
#	PILON_REF="/proj/data9/Loss_of_function_mutations/05_PilonAssembly_v_v112/P8511_101_gatk/P8511_101_sorted_rdgrp_nodups_fixmate_svcalls.vcf"
	...
	echo -e "zcat "$i" | java -jar /usr/local/packages/snpEff/SnpSift.jar filter \"(( QUAL >= 100 ) && (DP >= 30))\" > "$filename"_filt.vcf\n" >> $filename"_script.sge"
