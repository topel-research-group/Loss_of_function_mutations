...
for i in /proj/data9/Loss_of_function_mutations/01_Mapping/P*/; do
	...
	touch $dirname"_D_script.sge"
	echo -e "#$ -cwd\n#$ -q high_mem\n#$ -S /bin/bash\n#$ -pe mpi 1\n" > $dirname"_D_script.sge"
	...
	echo -e "bgzip -c "$dirname"/"$dirname"_D_S_marinoi_v1.1.2.vcf > "$dirname"/"$dirname"_D_S_marinoi_v1.1.2.vcf.gz" >> $dirname"_D_script.sge"

	#submit, evil laugh
	qsub $dirname"_D_script.sge"	
done
