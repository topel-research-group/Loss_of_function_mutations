...
for i in /proj/data9/Loss_of_function_mutations/00_Data/P*; do
	...
	echo -e "bowtie2 -x /proj/data26/Skeletonema_marinoi_genome_project/12_remove_redundancy/Fake_Primary_Removal/Smar_v1.1.2 --no-unal -p 8 -1 "$TMATE1" -2 "$TMATE2" -S "$dirname"/"$dirname".sam\n" >> $dirname"_script.sge"
