...
for i in /proj/data9/Loss_of_function_mutations/02_SV_calling_Pindel_Lov/P*/*_D; do
	...
	echo -e "mkdir -p "$dirname"_D\n" >> $dirname"_script.sge"
        ...
        echo -e "pindel2vcf -p /proj/data9/Loss_of_function_mutations/02_SV_calling_Pindel_Lov/"$dirname""/""$dirname"_D -r "$v112_FASTA" -R S_marinoi_v1.1.2 -d "$date_lbl" -v "$dirname"_D/"$dirname"_D_S_marinoi_v1.1.2.vcf\n" >> $dirname"_script.sge"

        echo -e "sed 's/^0/Sm_0/g' "$dirname""_D/""$dirname"_D_S_marinoi_v1.1.2.vcf > "$dirname""_D/""$dirname"_D_ChrNamesCor.vcf\n" >> $dirname"_script.sge"

	echo -e "bedtools intersect -v -b "$Pilon_D_REF" -a "$dirname"_D/"$dirname"_D_ChrNamesCor.vcf -sorted > "$dirname"_D/"$dirname"_D_PilonMskd.vcf" >> $dirname"_script.sge"
